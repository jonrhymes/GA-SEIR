...
// gulp.task('random', (cb) => {
// 	if(num === 1){
// 		console.log('You\'re so cool')
// 		cb();
// 	} else {
// 		console.log('Cooler than the other side of the pillow');
// 		cb();
// 	}
// });
...
gulp.task('default', (cb) => {
	browserSync.init({
		server: './public',
		notify: true,
		open: true //change this to true if you want the browser to open automatically, serves the files in the public folder
	});
	exec('npm run dev:webpack', function(err, stdout, stderr) { // Guaranteed to run at least one time. Makes code readable for other people, old school browsers
		console.log(stdout);
		console.log(stderr);
		cb(err);
	});
	gulp.watch('./src/scss/**/*',  gulp.task('styles')); // checks for changes inside our sass files
	gulp.watch('./src/components/**/*', gulp.task('webpack')); // checks for changes inside our components file
	gulp.watch('./src/main.js',gulp.task('webpack'))
	// checks for changes inside our main.js file
	gulp
		.watch([
			'./public/**/*',
			'./public/*',
			'public/js/**/.#*js',
			'public/css/**/.#*css'
		]) // reload when anything in public changes
		.on('change', reload);
		cb()
});
